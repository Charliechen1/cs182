...
def batchnorm_backward(dout, cache):
    ...
    N, D = x.shape
    dx_norm = dout * gamma
    dvar = np.sum(dx_norm * (x - mu), axis=0) * -0.5 * (var + eps) ** -1.5
    dmu = np.sum(dx_norm * -(var + eps) ** -0.5, axis=0) + dvar * np.mean(-2. * (x - mu), axis=0)

    dx = (dx_norm * (var + eps) ** -0.5) + (dvar * 2 * (x - mu) / N) + (dmu / N)
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx_norm = dout * gamma
    dvar = np.sum(dx_norm * (x - mu), axis=0) * -0.5 * std_inv**3
    ...
    for i in range(N):
        ...
        for h_ in range(H_): 
            for w_ in range(W_):
                ...
                w_end = w_start + pw
                for c in range (C):
                    window = data_point[c, h_start:h_end, w_start:w_end]
                    out[i, c, h_, w_] = np.max(window)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    cache = (x, pool_param)
        ...
        for h_ in range(H_): 
            for w_ in range(W_):
                ...
                for c in range(C):
                    ...
                    # the derivative should be just 1 or 0, so we use a mask
                    dx[i, c, h_start:h_end, w_start:w_end] += mask * dout[i, c, h_, w_]
...
def spatial_batchnorm_backward(dout, cache):
    ...
    dx_tmp, dgamma, dbeta = batchnorm_backward_alt(dout_reshaped, cache)
